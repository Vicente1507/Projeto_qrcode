...
const geeks = ['Torno Schaublin', 'Torno', 'Pedro Ignições', 'Paulin Mecânico'];
...
server.get('/crud/:index', checkGeekInArray, (req, res) => {
  return res.json(req.geek);
})

server.post('/crud', checkGeekExists, (req, res) => {
...
server.put('/crud/:index', checkGeekInArray, checkGeekExists, (req, res) => {
...
server.delete('/crud/:index', checkGeekInArray, (req, res) => {
  const { index } = req.params; // recupera o index com os dados
...
function checkGeekExists(req, res, next) {
    if (!req.body.name) {
    return res.status(400).json({ error: 'Nome da máquina é necessario' });
    ...
    const geek = geeks[req.params.index];
    if (!geek) {
    return res.status(400).json({ error: 'Maquina não existente' });
    } // checa se o Geek existe no array, caso negativo informa que o index não existe no array
    ...
    server.post('/crud', checkGeekExists, (req, res) => {
    const { name } = req.body; // assim esperamos buscar se a propriedade name foi informada corretamente
    geeks.push(name);
